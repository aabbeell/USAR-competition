...
public class BFS {
    ...
    public static Path search(Cell start, Cell target, Map map)
    {
        List<Cell> al = new ArrayList<Cell>();
        al.add(target);
        return search(start,al, map);
    }

    public static Path search(Cell start, List<Cell> targets, Map map)
    {
        ...
        while((cellInList(targets,visited))==null && !queue.isEmpty())
        {
            ...
            if(true)
            {
                ...
                if(c.getX()<map.getWidth()-1) neighbours[2] = map.getCell(c.getX(),c.getY()-1);
                if(c.getY()<map.getHeight()-1) neighbours[3] = map.getCell(c.getX(),c.getY()+1);
            }
    ...
    //TODO return with reachable cells (with undiscovered possibles too)
    public static List<Cell> getReachables(Cell start, Map map)
    {
